...
const productService = {
  ...
  getProductById: async (id) => {
    try {
      ...
      // Ensure product has a createdAt date
      if (!product.createdAt) {
        console.warn('Product has no createdAt date:', product.id);
        product.createdAt = new Date().toISOString();
      }
      
      // Ensure we have complete category data if it exists
      if (product && product.categoryId && (!product.category || !product.category.slug)) {
        console.log('Fetching missing category data for product', product.id);
        try {
          ...
          if (categoryResponse?.data?.data?.category) {
            product.category = categoryResponse.data.data.category;
          }
        } catch (categoryError) {
          console.error('Error fetching complete category data:', categoryError);
        }
      }
  ...
  getSellerProducts: async (username, status = 'ACTIVE') => {
    try {
      ...
      // Add defensive check
      if (!response.data || !response.data.data || !response.data.data.sellerProducts) {
        console.warn('Unexpected response format or empty data from sellerProducts query:', response);
        return [];
      }
      
      const products = response.data.data.sellerProducts;
      console.log(`Retrieved ${products.length} ${status} products for user ${username}:`, products);
      
      // Ensure each product has a defined soldQuantity
      return products.map(product => {
        if (product.soldQuantity === undefined || product.soldQuantity === null) {
          console.log(`Setting default soldQuantity for product ${product.id}`);
          product.soldQuantity = status === 'SOLD' ? 1 : 0;
        }
        // Add an empty reviewSummary object if needed
      ...
      console.error(`Error fetching ${status} products for seller ${username}:`, error);
      return [];
    }
  },
