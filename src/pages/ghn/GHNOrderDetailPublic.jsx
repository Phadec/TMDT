...
function GHNOrderDetailPublic() {
  ...
  // Lấy thông tin đơn hàng trực tiếp từ API
  useEffect(() => {
    const fetchOrderDetail = async () => {
      try {
        ...
        // Gọi API trực tiếp không qua adminServices
        const response = await axios.get(`http://localhost:8080/api/v1/admin/orders/${ORDER_ID}`, {
          headers: {
        ...
        if (err.response) {
          if (err.response.status === 401) {
            setError('Không có quyền truy cập. API yêu cầu xác thực.');
          } else if (err.response.status === 404) {
            setError('Không tìm thấy đơn hàng với ID: ' + ORDER_ID);
          } else {
            setError(`Lỗi API: ${err.response.status} - ${err.response.data?.message || 'Unknown error'}`);
          }
        } else if (err.request) {
        ...
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">
              API Endpoint: http://localhost:8080/api/v1/admin/orders/{ORDER_ID}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">Chi tiết đơn hàng GHN</h1>
...
export default GHNOrderDetailPublic;